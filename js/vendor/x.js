...
var colors = [
    new THREE.Color(0x5f85ff),
    new THREE.Color(0x0a0e2b),
    new THREE.Color(0x0d0f1e)];

var renderer = new THREE.WebGLRenderer({
    canvas: canvas,
    antialias: true
});
renderer.setPixelRatio(window.devicePixelRatio > 1 ? 1.8 : 1);
renderer.setSize(width, height);
renderer.setClearColor(0x22243a);
...
for (var i = 0; i < dotsAmount; i++) {
    ...
    vector.multiplyScalar(70 + (Math.random() - 0.5) * 5);
    vector.scaleX = 5;
...
function moveDot(vector, index) {
        ...
        tempVector.multiplyScalar((Math.random() - 0.8) * 0.2 + 1);
